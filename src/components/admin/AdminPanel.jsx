...
const AdminPanel = ({ onClose }) => {
  ...
  const handleUpdateLevel = async (userId, nuevoNivel) => {
    ...
    try {
      ...
      setUsuarios(
        usuarios.map((user) =>
          user.id === userId ? { ...user, userLevel: nuevoNivel } : user
        )
      );
  ...
  const handleBloquearUsuario = async (userId, email, bloqueado) => {
    ...
    if (window.confirm(`¿Estás seguro de ${accion} al usuario ${email}?`)) {
      ...
      try {
        ...
        alert(
          `Usuario ${bloqueado ? "desbloqueado" : "bloqueado"} exitosamente`
        );
      } catch (error) {
  ...
  const filteredUsers = usuarios.filter((usuario) => {
    ...
    const matchesLevel =
      filterLevel === "todos" || usuario.userLevel === filterLevel;
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center p-4">
      <div
        className="w-screen h-screen bg-black/50 absolute top-0"
        onClick={onClose}
      ></div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        Registro: {formatDate(usuario.createdAt)} | Último
                        login: {formatDate(usuario.lastLogin)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-medium ${
                        userLevels.find((n) => n.key === usuario.userLevel)
                          ?.bg || "bg-gray-100"
                      } ${
                        userLevels.find((n) => n.key === usuario.userLevel)
                          ?.color || "text-gray-600"
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleUpdateLevel(usuario.id, e.target.value)
                      }
                    ...
                    {usuario.id !== currentUser.uid && (
                      <button
                        onClick={() =>
                          handleEliminarUsuario(usuario.id, usuario.email)
                        }
                        disabled={updatingUser === usuario.id}
