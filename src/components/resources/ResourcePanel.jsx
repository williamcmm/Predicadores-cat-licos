...
export default function ResourcePanel() {
  ...
  const handleSavePastedText = () => {
    ...
    setSearchResults(prevResults => {
      ...
      const existingCategoryIndex = newResults.findIndex(c => c.category === PERSONAL_RESOURCE_CATEGORY);
 ...
 return () => document.removeEventListener('mousedown', handleClickOutside);
  ...
  const mapCategory = (incoming) => { if (!incoming) return incoming; const up = (incoming || '').toUpperCase(); const categoryNames = categories.map(c => c.name); for (const c of categoryNames) if (c === up) return c; for (const c of categoryNames) { const key = c.split(' ')[0]; if (up.includes(key) || c.includes(up.split(' ')[0])) return c; } return incoming; };
  const getFoundForCategory = (catName) => { const list = searchResults || []; return list.find(c => { const resultCatName = (c.category || '').toUpperCase(); if (!resultCatName) return false; if (resultCatName === catName) return true; const key = catName.split(' ')[0]; if (resultCatName.includes(key)) return true; const catToken = resultCatName.split(' ')[0]; if (catName.includes(catToken)) return true; return false; }) || null; };
  const handleToggleCategory = (cat) => { setUserToggledCategories(prev => ({ ...prev, [cat]: !prev[cat] })); };
  const handleSearch = async (termToSearch) => { if (isLoading) return; abortControllerRef.current = new AbortController(); const signal = abortControllerRef.current.signal;
 ...
 const activeCategories = categories.filter(c => c.active && c.name !== PERSONAL_RESOURCE_CATEGORY).map(c => c.name);
 ...
 if (isSuggesting) setIsSuggesting(false);
  ...
  const handleStopSearch = () => { if (abortControllerRef.current) abortControllerRef.current.abort(); setIsLoading(false); };
  ...
  const onSuggestionClick = async (suggestion) => { const clean = (suggestion || '').replace(new RegExp('https?://(\S+)'), '').trim(); setSearchTerm(clean); setShowDropdown(false); await handleSearch(clean); };
  ...
  const handleGenerateSermon = async () => {
    ...
    if (!filteredResults || filteredResults.length === 0 || filteredResults.every(c => !c.resources || c.resources.length === 0)) {
      alert("Por favor, activa y busca en algunas categorías, o añade un recurso personal antes de generar un sermón.");
      return;
    }

    const isStillLoading = Object.values(categoryLoading).some(status => status === true);
    if (isStillLoading) {
      const confirmation = window.confirm("La búsqueda de recursos aún no ha terminado.\n\nGeneraremos el sermón solo con los recursos encontrados hasta el momento.\n\n¿Desea continuar?");
    ...
    try {
      ...
      if (sermonData) {
        window.dispatchEvent(new CustomEvent('insertSermonIntoEditor', { detail: sermonData }));
        try { localStorage.setItem('lastGeneratedSermon', JSON.stringify(sermonData)); } catch (e) {
  ...
  const hasContent = () => searchResults && searchResults.some(c => c.resources && c.resources.length > 0);
                ...
                <div key={cat.name} className={`border rounded p-3 transition-opacity ${!cat.active ? 'opacity-50' : 'bg-gray-50'}`}>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                    ...
                    <div className="flex items-center space-x-2 self-end sm:self-center">
                      ...
                      {count > 0 && <button onClick={() => handleToggleCategory(cat.name)} className="text-blue-600 hover:underline text-sm">{isExpanded ? 'Cerrar' : 'Abrir'}</button>}
                    </div>
                  </div>
                          ...
                          return (
                            <div key={rid} className="border-b pb-2 mb-2 last:border-b-0">
                              ...
                              <div className="text-sm text-gray-700">
                                {resource.excerpt ? <p className={expandedResources[rid] ? '' : 'line-clamp-3'}>{resource.excerpt}</p> : (resource.content && resource.content.length > 0 ? <p className={expandedResources[rid] ? '' : 'line-clamp-3'}>{resource.content[0]}</p> : <p className="text-sm text-gray-500">Sin excerpt disponible.</p>)}
                                {(resource.excerpt || (resource.content && resource.content[0])) && <button className="text-sm text-blue-600 mt-1" onClick={() => handleToggleReadMore(rid)}>{expandedResources[rid] ? 'Leer menos' : 'Leer más'}</button>}
                              </div>
                              {expandedResources[rid] && resource.content && resource.content.length > 1 && <div className="mt-2 space-y-3">{resource.content.slice(1).map((p, pi) => <p key={pi}>{p}</p>)}</div>}
                            </div>
                          );
                        })
                      )}
