...
export async function searchResourcesProgressive(query, onCategory, categories = null, signal) {
  ...
  for (const cat of cats) {
    ...
    if (cat === 'CITAS BÍBLICAS RELEVANTES') {
      ...
      basePrompt = `\n      Actúas como un asistente de investigación católico experto.\n      Tu tarea es encontrar recursos relevantes para el tema: \"${query}\".\n      Debes enfocarte EXCLUSIVAMENTE en la categoría: \"${cat}\".\n\n      Instrucciones de formato de salida:\n      - La respuesta DEBE ser un objeto JSON válido.\n      - El objeto JSON debe contener una única clave llamada \"resources\".\n      - El valor de \"resources\" debe ser un array de objetos.\n      - Cada objeto en el array debe tener las siguientes claves: \"title\", \"source\", y \"content\".\n      - \"title\": Un título descriptivo para el recurso.\n      - \"source\": La fuente del recurso (ej: \"Catecismo de la Iglesia Católica 2267\", \"Juan Pablo II, Evangelium Vitae\").\n      - \"content\": Un array de strings, donde cada string es un párrafo del contenido del recurso.\n\n      Requisitos de contenido:\n      - Encuentra entre 3 y 7 recursos relevantes si es posible.\n      - El contenido debe ser sustancial y directamente relacionado con el tema y la categoría.\n      - Utiliza únicamente fuentes católicas ortodoxas y confiables.\n\n      Genera el JSON ahora.\n      `;
    }
    try {
...
export async function generateGeneralSuggestions() {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      throw new Error('La solicitud de sugerencias tardó demasiado. Inténtalo de nuevo.');
    }
    // Check for common API errors (e.g., rate limiting)
    if (e.message && e.message.includes('429')) {
      throw new Error('Límite de solicitudes alcanzado. Por favor, espera un momento.');
    }
    throw new Error('No se pudieron obtener las sugerencias. Revisa la consola para más detalles.');
  }
}
